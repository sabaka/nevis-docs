...
class DocumentRepositoryIntegrationTest {
  ...
  private DocumentRow readDocument(UUID id) {
    return jdbcClient
        .sql("select client_id, title, content, created_at from document where id = :id")
        .param("id", id)
        .query(
            (rs, _) ->
                new DocumentRow(
  ...
  private record DocumentRow(UUID clientId, String title, String content, Instant createdAt) {}
}
